...
def ai_translate(text: str, api_key: str) -> str:
    ...
    return re.sub(r'\b[Mm]e\b', fix_pronoun_casing, translated_text)
...
with st.sidebar:
    ...
    # Check for environmental fallback key first, otherwise blank
    ...
    * **No Profit:** If you choose to use a paid tier, that money is a direct infrastructure service fee paid directly to Google Cloud to keep the server computing. **Zero percent of this goes to the developer.**
...
if st.button("Translate", type="primary"):
    if not english_text.strip():
        ...
        with st.spinner("Processing regional dialects..."):
            ...
            if user_key:
                try:
                    result = ai_translate(english_text, user_key)
